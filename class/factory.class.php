...
class Factory extends CommonObject
{
	...
	function importComposition($xml)
	{
		...
		if (isset($tblfields['productid'])) {
			$tblfields = array($tblfields);
		} // Fix single-element array flattening
	...
	function getNomUrl($withpicto = 0)
	{
		...
		if ($withpicto)
			$result .= ($lien . $linkclose . img_object($label, $picto) . $lienfin);
		if ($withpicto && $withpicto != 2)
			$result .= ' ';
		if ($withpicto != 2)
			$result .= $lien . $linkclose . $this->ref . $lienfin;
		return $result;
	}
		...
		if ($withpicto) {
			if ($tmpproduct->type == 0)
				$result .= ($lien . img_object(
					$langs->trans("ShowProduct") . ' ' . $productref,
					'product'
				) . $lienfin . ' ');
			if ($tmpproduct->type == 1)
				$result .= ($lien . img_object(
					$langs->trans("ShowService") . ' ' . $productref,
					'service'
				) . $lienfin . ' ');
		}
		...
		if ($resql) {
			...
			if ($nump) {
				...
				while ($i < $nump) {
					...
					$this->sousprods["fab"][$obj->id] = array(
						0 => $obj->id,
						1 => $obj->qty_unit,
				...
				if ($res) {
					...
					while ($rec = $this->db->fetch_array($res)) {
						$idmv = $mouvP->livraison(
							...
							$rec['qty_planned'],
							$rec['price'],
							$langs->trans("UsedforFactory", $this->ref),
							$this->date_start_made
						);
